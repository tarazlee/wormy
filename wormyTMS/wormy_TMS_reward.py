...
MINTIME = 2000
SUB_NUM = raw_input("Please enter your subject number:")
TMS_SESSION = raw_input("Please enter TMS session (D or V):")
# name/location of reward sequence numpy file
REWARDFILE = 'reward_schedules/%s_rewards_and_distances.csv' % SUB_NUM 
JITTERFILE = 'TMS_jitters.npy' # name/location of jitter sequence numpy file
READYFILE = 'TMS_readytimes.npy' #name/location of ready time sequence numpyfile
...
NUMREWARD = 162 # how many reward trials to do (162 if m-sequence for 3 reward values)
BLOCKLENGTH = 27
...
def main():
    ...
    doStaircase = 'no'
    if doStaircase == 'no':
        exp_day = raw_input("Enter day number:")
        threshold = raw_input("Enter apple size:")
        maxTime = int(raw_input("Enter maximum time:"))
        
    
    if MSEQ == 'yes':
        trial_schedule = pd.read_table(REWARDFILE)
        rewardSchedule = trial_schedule.reward
        distanceSchedule = trial_schedule.distance
        jitterSchedule = np.load(JITTERFILE)
        readySchedule = np.load(READYFILE)
    else:
        print "something wrong with MSEQ variable"
        1/0
    ...
    for i in range(numTrials):
        ...
        if doStaircase == 'no':
            reward = rewardSchedule[i]
            distance = distanceSchedule[i]
        else: reward = 0
        ...
        trial_outcome, start_locX, start_locY, apple_loc, startSide, \
            ...
            = runGame(distance, size,i, str(reward),sidesSchedule,maxTime, readySchedule) 
        # fill in output data
        ...
        data = {'startX': worm_locX, 'starty': worm_locY, 'appleX': applesX, 
                ...
                'runNum': blockNums, 'TMS_loc': TMS_locs, 'exp_day' : exp_days, 'distance': distances
                }
        frame = DataFrame(data)
        # write temporary output in case something crashes . . .
        frame.to_csv('temp_output_files/%s_day_%s_%s_temp_reward_output.csv' % \
                     (subject_number, exp_day, TMS_SESSION), sep= '\t')  
        np.save('temp_output_files/'+subject_number +'_day_' + exp_day+
                '_'+ TMS_SESSION +"_temp_reward_trajectories.npy", trajectories)
    # save output panda as CSV and give feedback to user

    frame.to_csv('output_files/%s_%s_reward.csv' % (subject_number, TMS_SESSION), sep= '\t') #write output
    ...
    np.save('output_files/'+ subject_number + '_day_' + exp_day+'_'+TMS_SESSION
            + "_reward_trajectories.npy", trajectories)    
    terminate()

# Run one trial
# returns trial outcome, worm location, apple location, and length of trial in ms
def runGame(distance,targetSize, trialNum, rewardVal,startingSides, maxTime, readySchedule):
    ...
    appleCoords = makeSpacedApple(targetSize, side, wormCoords[HEAD], distance) # make apple the correct distance away
...
def getRandomWormLoc(side):
    x_range = range(MAXAPPLESIZE,CELLWIDTH/3) + range(2*CELLWIDTH/3,CELLWIDTH - MAXAPPLESIZE)
    y_range = range(MAXAPPLESIZE,CELLHEIGHT/3) + range(2*CELLHEIGHT/3,CELLHEIGHT - MAXAPPLESIZE)
    if side == LSIDE:
        startx = 5 #random.randint(5, CELLWIDTH - 6)
        starty = random.choice(y_range)
    elif side == BSIDE:
        startx = random.choice(x_range)
        starty = CELLHEIGHT - 5
    elif side == RSIDE:
        startx = CELLWIDTH - 5
        starty = random.choice(y_range)
