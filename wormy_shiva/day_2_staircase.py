...
REWARDVALUES = ['?', 5, 5, 20, 20]
SIDES = [LSIDE,BSIDE,RSIDE] # To randomly choose a starting side
...
TRAINING = 10 # how many training trials to do
NOTIMELIMIT = TRAINING
...
def main():
    ...
    doStaircase = 'yes' #raw_input("Staircase needed? (please answer 'yes' or 'no')")
    threshTime = int(raw_input("Enter maximum time:"))
    ...
    for i in range(numTrials):
        if i > TRAINING and doStaircase == 'yes': maxTime = threshTime
        ...
        trial_outcome, start_locX, start_locY, apple_loc, startSide, \
            trial_time, speedPress, steerPress = runGame(size,i, str(reward),sidesSchedule,maxTime) 
        # fill in output data
        ...
        if i < (NOTIMELIMIT) and doStaircase == 'yes': 
            training.append('yes')
            reversals.append(0)
        ...
        if trial_outcome == 3: 
            ...
            elif i >= NOTIMELIMIT and doStaircase == 'yes': # for speed staircasing
       ...
       # make output panda structure
        ...
        frame.to_csv('%s_temp_day2.csv' % subject_number, sep= '\t')
    frame.to_csv('%s_day2_training.csv' % subject_number, sep= '\t') #write output
    stairSize =[np.mean(reversalSizes)]
    stairSpeed = [maxTime]
    print 'staircased apple size is %s' % str(stairSize[0])
    print 'last speed deadline was %s' % str(maxTime)
    print 'staircased speed deadline is %s' % str(stairSpeed[0])
    stairData = {'staircased_size': stairSize, 'staircased_deadline': stairSpeed}
    stairFrame = DataFrame(stairData)
    stairFrame.to_csv('%s_day_2_params.csv' % subject_number, sep = '\t')
...
def runGame(targetSize, trialNum, rewardVal,startingSides, maxTime):
    ...
    if side == LSIDE: 
        ...
        direction = DOWN
    FPS = FPSSTART
    #SPEED = random.choice(SPEEDSTARTS)
    SPEED = 1
    ...
    while True: # main game loop
        ...
        if curr_time < WAITTIME: 
            ...
            if checkForKeyPress():
                pygame.event.get()
        elif curr_time < WAITTIME + READYTIME:
            ...
            if curr_time > maxTime and (trialNum > NOTIMELIMIT or rewardVal !='0'):
